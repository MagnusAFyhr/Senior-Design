...
def ATR(df, n):  
    ...
    while i < df.index[-1]:  
        ...
        #above line is original. Below is adjusted for depreciation errors
        TR = max(df.loc[i + 1, 'High'], df.loc[i, 'Close']) - min(df.loc[i + 1, 'Low'], df.loc[i, 'Close'])
        TR_l.append(TR)  
        i = i + 1  
    TR_s = pd.Series(TR_l)  
    #ATR = pd.Series(pd.ewma(TR_s, span = n, min_periods = n), name = 'ATR_' + str(n))
    #Error encountered due to line below. Line above used ewma which has since been depreciated
...
def KELCH(df, n):  
    ...
    KelChM = pd.Series(((df['High'] + df['Low'] + df['Close']) / 3).rolling(n).mean(), name = 'KelChM_' + str(n))
    #KelChU = pd.Series(pd.rolling_mean((4 * df['High'] - 2 * df['Low'] + df['Close']) / 3, n), name = 'KelChU_' + str(n))  
    KelChU = pd.Series(((4 * df['High'] - 2 * df['Low'] + df['Close']) / 3).rolling(n).mean(), name = 'KelChU_' + str(n))
    #KelChD = pd.Series(pd.rolling_mean((-2 * df['High'] + 4 * df['Low'] + df['Close']) / 3, n), name = 'KelChD_' + str(n))  
    KelChD = pd.Series(((-2 * df['High'] + 4 * df['Low'] + df['Close']) / 3).rolling(n).mean(), name = 'KelChD_' + str(n))
    df = df.join(KelChM)  
    df = df.join(KelChU)  
    df = df.join(KelChD)  
    return df
...
data = PPSR(data)
data = STOK(data)
data = STO(data, 10, 10, nS=1)
data = TRIX(data, 10)
data = ADX(data, 10, 10)
data = MACD(data, 8, 9)
data = MassI(data)
data = Vortex(data, 10)
data = KST(data, 10, 10, 10 ,15, 10, 15, 20, 30)
data = RSI(data, 10)
data = TSI(data, 25, 13)
data = ACCDIST(data, 10)
data = Chaikin(data)
data = MFI(data, 14)
data = OBV(data, 10)
data = FORCE(data, 13)
